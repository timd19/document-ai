    ...
    try:  
        ...
        if history:  
            for entry in history:  
                # Each entry should be either a (user, assistant) tuple/list, or a dict.  
                if isinstance(entry, (list, tuple)) and len(entry) == 2:  
                    ...
                    # Expecting keys 'role' and 'content' (be defensive)  
                    role = entry.get("role")  
...
def upload_document(file_path, session_id):  
    ...
    with open(file_path, "rb") as f:  
        files = {  
            # third element (content_type) is optional; FastAPI will sniff it  
            "file": (filename, f)  
        }  
